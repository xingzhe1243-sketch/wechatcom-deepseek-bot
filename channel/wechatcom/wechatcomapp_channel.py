...
class WechatComAppChannel(ChatChannel):
    ...
    def __init__(self):
        super().__init__()
        # 同时兼容嵌套格式 (wechatcom_app.xxx) 和扁平格式 (wechatcom_xxx)
        wc_cfg = conf().get("wechatcom_app", {})
        self.corp_id = wc_cfg.get("corp_id") or conf().get("wechatcom_corp_id", "")
        self.secret = wc_cfg.get("corpsecret") or conf().get("wechatcomapp_secret", "")
        agent_id_raw = wc_cfg.get("agent_id") or conf().get("wechatcomapp_agent_id", 0)
        self.agent_id = int(agent_id_raw) if agent_id_raw else 0
        self.token = wc_cfg.get("token") or conf().get("wechatcomapp_token", "")
        self.aes_key = wc_cfg.get("encoding_aes_key") or conf().get("wechatcomapp_aes_key", "")
