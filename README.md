...
本仓库是 [CowAgent](https://github.com/zhayujie/CowAgent) 的定制 Fork，专为企业微信 + DeepSeek 机器人优化：

- ✅ **修复硬编码人设不生效问题**：修改 `models/deepseek/deepseek_bot.py`，强制注入系统人设，无视 `system_prompt` 字段失效的 bug
- ✅ **兼容嵌套 config.json 格式**：修改 `channel/wechatcom/wechatcomapp_channel.py`，同时支持嵌套格式 (`wechatcom_app.xxx`) 和扁平格式 (`wechatcom_xxx`)
- ✅ **移除所有敏感信息**：config.json 已加入 .gitignore，提供 `config-template.json` 模板
- ✅ **完整搭建手册**：[docs/setup.md](docs/setup.md) — 从零开始的图文教程，适合零基础小白
...
# 复制 config-template.json 为 config.json，填写你的真实配置
cp config-template.json config.json
# 修改 models/deepseek/deepseek_bot.py 中 my_personality 字段设置人设
python app.py
```
...
| 文件 | 修改说明 |
|------|---------|
| `models/deepseek/deepseek_bot.py` | 在 `reply_text` 函数中强制注入系统人设 |
| `channel/wechatcom/wechatcomapp_channel.py` | 兼容嵌套格式的 config.json 读取 |
| `docs/setup.md` | 完整的从零搭建手册 |
| `config-template.json` | 企业微信 + DeepSeek 配置模板 |
